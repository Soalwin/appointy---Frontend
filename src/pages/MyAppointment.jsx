...
const MyAppointments = () => {
  ...
  const appointmentRazorpay = async (appointmentId) => {
    ...
    const stripe = await loadStripe('pk_test_51SeWycRqEAjs26HszB9jfkZ5heV9VNX12kikJGWtBbxoxGthlLoY8bJ6TKHJmhTzLrVERIqK1RVBjM2c2cfbC1rI00mtfHWOHp');
      ...
      const  result  = await axios.post(backendUrl + '/api/user/payment-razorpay', { appointmentId }, { headers: { token } })
              console.log(result);
        ...
        {appointments.map((item, index) => (
          <div key={index} className='grid grid-cols-[1fr_2fr] gap-4 sm:flex sm:gap-6 py-4 border-b'>
            ...
            <div className='flex flex-col gap-2 justify-end text-sm text-center'>
              ...
              {!item.cancelled && !item.payment && !item.isCompleted && payment === item._id && <button onClick={() => appointmentRazorpay(item._id)} className='text-[#696969] sm:min-w-48 py-2 border rounded hover:bg-gray-100 transition-all duration-300 flex items-center justify-center'> Continue to Stripe</button>}
